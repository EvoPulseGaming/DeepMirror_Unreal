...
UCVUMat::UCVUMat() {
  ...
  UE_LOG(DeepMirrorPlugin, Verbose, TEXT("Default Constructed"));
#endif
};

UCVUMat::UCVUMat(const cv::UMat &m_) : m(m_) {
#if CV_ENABLE_INSTANCE_TRACKING
  UE_LOG(DeepMirrorPlugin, Verbose, TEXT("Constructed w/ cv::Mat constructor"));
#endif
};
...
UCVUMat::~UCVUMat() {
#if CV_ENABLE_INSTANCE_TRACKING
  UE_LOG(DeepMirrorPlugin, Verbose, TEXT("Destroyed"));
#endif
};
  ...
  try {
    ...
    switch (m.type()) {
      ...
      case CV_8UC4:
        ...
        break;
      default:
        UE_LOG(DeepMirrorPlugin, Warning, TEXT("It seems that the OpenCV type is not supported!"));
        return;
    }
    ...
    UE_LOG(DeepMirrorPlugin, Warning, TEXT("Function %s: Caught OpenCV Exception: %s"), TEXT(__FUNCTION__),
...
void UCVUMat::ToTexture(UTexture2D *&Texture, bool Resize) {
  ...
  if (m.total() == 0) {
    UE_LOG(DeepMirrorPlugin, Error, TEXT("Cannot upload an empty matrix!"));
    return;
  }
  ...
  try {
    ...
    if (m.cols != Texture->GetSizeX() || m.rows != Texture->GetSizeY()) {
      if (Resize) {
        ...
        UE_LOG(DeepMirrorPlugin, Warning, TEXT("Render target size does not match and resize=false!"));
        return;
      }
    }
...
void UCVUMat::ToVolumeTexture(UVolumeTexture *&VolumeTexture) {
  ...
  if (m.channels() != 1) {
    UE_LOG(DeepMirrorPlugin, Error, TEXT("Channel mismatch: Volume has to be single-channel!"));
    return;
  }
  if (m.dims != 3) {
    UE_LOG(DeepMirrorPlugin, Error, TEXT("Dimensionality mismatch: cv::UMat has to contain a 3D image!"));
    return;
  }
  if (m.elemSize() != 1) {
    UE_LOG(DeepMirrorPlugin, Error, TEXT("Pixel size mismatch: cv::UMat has to be 1 byte per pixel!"));
    return;
  }
  ...
  */
  {
    UE_LOG(DeepMirrorPlugin, Warning, TEXT("Created a new texture!"));
...
void UCVUMat::FromTexture2D(UTexture2D *Texture, UCVUMat *&Mat) {
  if (!ensure(Texture != nullptr)) {
    UE_LOG(DeepMirrorPlugin, Error, TEXT("The given texture is empty!"));
    return;
  }
  ...
  if (!ensure(cvFormat != -1)) {
    UE_LOG(DeepMirrorPlugin, Error, TEXT("Given texture has a currently unsupported format!"));
    return;
  }
